...
#include "HidHideControl.h"
#include "DllUtilities.h"

// Forward declaration for VR integration
...
#define XUSB_GAMEPAD_CUSTOM_RIGHT_TRIGGER 0x0111
#define XUSB_GAMEPAD_CUSTOM_LEFT_TRIGGER  0x0222
...
const std::unordered_map<std::string, WORD> unrealKeyMap = {
    ...
    {"^", VK_OEM_6},
    {"U_AccentGrave", VK_OEM_3}, 
    {"Asterisk", VK_OEM_5},       
    {"Dollar", VK_OEM_1},
    {"RightParenthesis", VK_OEM_4},
    {"²", VK_OEM_7},
    {"<", VK_OEM_102}
};
...
const std::unordered_map<std::string, WORD> xusb_gamepadButtonMap = {
    ...
    {"Gamepad_LeftTrigger", XUSB_GAMEPAD_CUSTOM_LEFT_TRIGGER},
    {"Gamepad_RightTrigger", XUSB_GAMEPAD_CUSTOM_RIGHT_TRIGGER},
...
const std::unordered_map<std::string, WORD> ds4_gamepadButtonMap = {
    ...
    {"Gamepad_LeftTrigger", DS4_BUTTON_TRIGGER_LEFT},
    {"Gamepad_RightTrigger", DS4_BUTTON_TRIGGER_RIGHT},
...
const std::unordered_map<std::string, std::vector<WORD>> defaultBinding = {
    ...
    {"oppugno", {'W'}},
    {"alohomora", {'F'}},
    {"protego contra", {'R', VK_LBUTTON}},
    {"episkey", {'G'}},
    {"apperta liber", {VK_ESCAPE}},
    {"finite liber", {VK_ESCAPE}},
    {"apperta mappa", {'M'}},
    {"apperta sacculus", {'L'}},
    {"apperta vestarium", {'I'}},
    {"apperta meritas", {'U'}},
    {"apperta codex", {'J'}},
    {"apperta compendium", {'Y'}},
    {"apperta literae", {'O'}},
    {"apperta facultates", {'N'}},
    {"apperta configuratio", {'P'}},
    {"apperta incantatem", {'T'}},
};
...
const std::unordered_map<std::string, std::vector<WORD>> defaultXUSBBinding = {
    ...
    {"protego contra", { XUSB_GAMEPAD_Y, XUSB_GAMEPAD_CUSTOM_RIGHT_TRIGGER}},
    {"episkey", {XUSB_GAMEPAD_DPAD_DOWN}},
    {"apperta liber", {XUSB_GAMEPAD_START}},
    {"finite liber", {XUSB_GAMEPAD_START}},
    {"apperta mappa", {XUSB_GAMEPAD_BACK}}, // 'M'
    {"apperta sacculus", {'L'}},
    {"apperta vestarium", {'I'}},
    {"apperta meritas", {'U'}},
    {"apperta codex", {'J'}},
...
const std::unordered_map<std::string, std::vector<WORD>> defaultDS4Binding = {
    ...
    {"protego contra", { DS4_BUTTON_TRIANGLE, DS4_BUTTON_TRIGGER_RIGHT}},
    {"episkey", {DS4_BUTTON_DPAD_SOUTH}},
    {"apperta liber", {DS4_BUTTON_OPTIONS}},
    {"finite liber", {DS4_BUTTON_OPTIONS}},
    {"apperta mappa", {DS4_BUTTON_SHARE}}, // 'M'
    {"apperta sacculus", {'L'}},
    {"apperta vestarium", {'I'}},
    {"apperta meritas", {'U'}},
    {"apperta codex", {'J'}},
...
const std::vector<std::vector<std::string>> spell_equivalences = {
    {"crucio", "endoloris"},
    {"depulso", "repulso"},
    {"impero", "imperio"},
    {"confundo", "confundus"},
...
const std::vector<std::string> special_menu_list = {
...
const std::vector<std::string> special_spell_list = {
    "apperta sacculus",
    "apperta vestarium",
    "apperta meritas",
    "apperta codex",
...
{
public:
	Keybinder(std::string conf_path, Config& cfg);
	~Keybinder();


    void selectingGamepad(Config& cfg);
    void startRedirectingController();
    void stopRedirectingController();
    void restoreController();

    void localizeGameBindings();
    std::unordered_map<std::string, std::vector<WORD>> loadGameBindings();
    std::unordered_map<std::string, std::vector<std::string>> buildReccurentSpellMap();
    void loadConfBindings(const std::string& conf_path, const std::unordered_map<std::string, std::vector<WORD>>& game_bindings);

    WORD keyToBind(const std::string& key, const std::string& actionName);
    bool updateBinding(std::unordered_map<std::string, std::vector<WORD>>& binding, const std::string& actionName, const std::string& key);
    
    auto findInPrincipalBindings(const std::string& word);
...
private:
    ...
    std::vector<std::pair<std::string, std::vector<WORD>>> _principal_bindings;
	std::unordered_map<std::string, std::vector<WORD>> _secondary_bindings; // _principal_bindings, 
    std::string _conf_path, _game_binding_path;
    std::size_t _conf_hash, _game_binding_hash;
    std::vector<std::string> _user_binding_txt;

    std::vector<std::wstring> _hidHide_blocked_paths;

    // --- Multi language spell mapping --- //
    std::unordered_map<std::string, std::vector<std::string>> _recurrent_spells_mapping;

    std::unique_ptr<HidHideControl> _hidHide;
    std::unique_ptr<InputRedirector> _inputRedirector;
    std::unique_ptr<VRInputHandler> _vrInputHandler;
    LPDIRECTINPUT8 _directInput;
    DIDEVICEINSTANCE _selectedController;
};
